...
This repository is a fork of the `visdat` R package for visualising data. 

The purpose of the fork primarily is to extend the original package's functionality to be more seamless for (multiple) time series data. 
In particular, it aims to extend all existing package functionality to 
- ts/mts
- zoo
- xts
- tsibble
- data.frame, with a suitable time index column

It should rely on the following dependencies
- zoo
- xts
- tsibble
- tsbox, to handle conversion between the different formats.

Assume that the time series data is formatted to be T x N in dimensions. 
Notice that time series data has some specific requirements:
...
**Universal Agents** apply to all stages regardless of context. **Stage-Specific Agents** apply only to code within that phase of the lifecycle. 
Files in `analysis/`, `scripts/`, or `exploratory/` directories are governed by Analysis agents; files in `R/`, `src/`, and standard package directories are governed by Development and Review agents.
...
- Make all returns explicit, and their final returns values should be named with an ret_ prefix + something informative, usually its class name (e.g. `ret_df`, `ret_list`, `ret_xts`). 
NEVER use implicit returns or rely on the last evaluated expression as the return value, or name the return something uninformative, such as `out`
- Always assign functions at the top level, never inside functions
- Ensure all functions are explicit with all of their arguments. 
This is especially so for anonymous functions - they must not rely on external variables, or implicit outer scope variables. 
The only exception to this are for wrapper functions, which need to allow users to pass down potentially many arguments
...
- This repository uses workspace management, e.g. .Rproj or .code-workspace. As such, always use relative file paths; NEVER use absolute paths. 
