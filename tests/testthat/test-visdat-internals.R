...
suppressWarnings({
  test_old_gather <- dplyr::mutate(typical_data, rows = seq_len(nrow(typical_data)))
  test_old_gather <- tidyr::gather_(test_old_gather,
                                    key_col = "variable",
                                    value_col = "valueType",
                                    gather_cols = names(test_old_gather)[-length(test_old_gather)])
  test_old_gather <- dplyr::arrange(test_old_gather, rows, variable, valueType)
})
...
test_that("fingerprint can deal with complete-cases list columns",{
  expect_equal(all(is.na(visdat:::fingerprint(dplyr::starwars$films))),FALSE)
})

test_that("fingerprint can count n/a in list columns",{
  expect_equal(sum(is.na(visdat:::fingerprint(dplyr::starwars$vehicles))),76)
})
