...
ts_to_df <- function(x) {
  if (is.null(x) || length(x) == 0L) {
    cli::cli_abort("Time series object must not be NULL or empty")
  }
  ...
  attrs_to_keep <- setdiff(names(orig_attrs), structural_attrs)

  if (length(attrs_to_keep) > 0) {
    attributes(series_df) <- utils::modifyList(attributes(series_df), orig_attrs[attrs_to_keep])
  }
