...
ts_to_df <- function(x) {
  ...
  # Normalize time index to Date for consistent scale_y_date() usage.
  # tsbox::ts_df() returns Date for ts/zoo/xts/tsibble, but POSIXct for mts.
  # Converting to Date ensures calendar-date rendering and user-friendly
  # axis labelling via scale_y_date().
  if (inherits(time_idx, "POSIXct") || inherits(time_idx, "POSIXt")) {
    time_idx <- as.Date(time_idx)
  }
  ...
  # Rename single "value" column to preserve original series name.
  # tsbox::ts_df() names single-series columns "value", losing the original
  # name. For mts, column names are preserved. For ts objects we can use
  # the series name from names() if set, otherwise fall back to "value".
  if (ncol(series_df) == 1L && names(series_df)[1L] == "value") {
    series_name <- NULL
    if (inherits(x, "ts") && !inherits(x, "mts")) {
      ...
      measure_vars <- setdiff(names(x), idx_var)
      if (length(measure_vars) == 1L) {
  ...
  # restore custom attributes preserved from the original object
  struct_attrs <- c("dim", "dimnames", "tsp", "class", "names", "row.names",
                    "index", "indexClass", "tclass", "tzone")
  save_attrs <- original_attrs[setdiff(names(original_attrs), struct_attrs)]
  attributes(series_df) <- utils::modifyList(attributes(series_df), save_attrs)
...
# ---------------------------------------------------------------------------
# Shared coordinate helpers for vis_* time-series plots
# ---------------------------------------------------------------------------

#' Replace geom_raster with geom_tile and set up time-series y-axis.
#'
#' Removes the first layer (geom_raster) and adds geom_tile with time mapped
#' to the y-axis, plus scale_y_date for proper date formatting.
#'
#' @param plot ggplot object
#' @param row_labels Date vector of time indices
#' @param x_aes Aesthetic mapping for x (default: variable)
#' @param fill_aes Aesthetic mapping for fill (default: valueType)
#' @return ggplot object with geom_tile and scale_y_date
#' @keywords internal
#' @noRd
...
#'
#' @param plot ggplot object
#' @param transpose whether to flip coordinates
#' @param x_label x-axis label (used as-is; swapped internally when transposed)
#' @param y_label y-axis label (used as-is; swapped internally when transposed)
#' @return ggplot object with coordinate transform and labels
#' @keywords internal
#' @noRd
vis_add_time_coords <- function(plot, transpose) {
  ret_plot <- plot
...
#' Add coordinate transformation for regular (non-time-series) vis_* plots.
#'
#' @param plot ggplot object
#' @param transpose whether to flip coordinates
#' @return ggplot object with coordinate transform and labels
#' @keywords internal
#' @noRd
vis_add_regular_coords <- function(plot, transpose) {
  ret_plot <- plot
