...
ts_to_df_patched2 <- function(x) {
  ...
  if (length(attrs_to_restore) > 0) {
    # It is important to merge with existing attributes, not replace them all,
    # as series_df now has names, class, row.names, and our newly added row_labels.
    attributes(series_df) <- modifyList(attributes(series_df), attrs_to_restore)
  }
...
cat("\n--- Testing ts_to_df_patched2 ---\n")
y <- ts_to_df_patched2(x)
print(str(y))
